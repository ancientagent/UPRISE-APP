...
import {getBandEvents} from '../../../state/selectors/UserProfile';
...
const EventTab = ({navigation}) => {
  ...
  return (
    <View style={styles.galleryTabView}>
      {BandEvents.length === 0 ? (
        <Text style={styles.eventTxt}>{strings('bandDetails.noEvent')}</Text>
      ) : (
        <FlatGrid
          maxItemsPerRow={1}
            ...
            <>
              ...
              <View style={{marginRight: 16}}>
                <View style={styles.eventDetailsTextView}>
                  ...
                  />
                  <Text style={styles.eventDetailsText}>{item.location}</Text>
                </View>
                <View style={styles.eventDetailsTextView}>
                  <SvgImage
                    iconName={clock}
      ...
      {BandEvents.length > 2 && (
        <Text
          ...
          onPress={() => navigation.navigate('FullEventView')}>
