...
import {getDiscoveryPopularBandsList} from '../../../state/selectors/UserProfile';
import Loader from '../../../components/Loader/Loader';

const AllPopularBands = props => {
  const {navigation} = props;
  const showLoading = useSelector(
    state => state.discoveryPopularBands.isWaiting,
  );
          ...
          <FlatGrid
            ...
            renderItem={({item}) => (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() =>
                  navigation.navigate('BandDetails', {bandId: item.id})
                }>
