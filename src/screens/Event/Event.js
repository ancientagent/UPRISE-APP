...
import {getHomeEvents} from '../../state/selectors/UserProfile';
import {
...
const Event = props => {
  ...
  console.log(
    '--- EVENTS COMPONENT: Redux state isWaiting ---',
    fullReduxState?.isWaiting,
  );
  console.log(
    '--- EVENTS COMPONENT: Redux state error ---',
    fullReduxState?.error,
  );
  console.log(
    '--- EVENTS COMPONENT: Redux state data ---',
    fullReduxState?.data,
  );

  useEffect(() => {
    console.log(
      '--- EVENTS COMPONENT: useEffect triggered - dispatching homeEventsSagaAction ---',
    );
    console.log(
      '--- EVENTS COMPONENT: About to dispatch homeEventsSagaAction ---',
    );
  ...
  const ListEmptyComponent = () => {
    ...
    return (
      <View style={{alignItems: 'center'}}>
        <Text style={styles.emptyTxt}>{strings('Event.emptyEvent')}</Text>
        <Image
  ...
  console.log(
    '--- EVENTS COMPONENT: EventData.length === 0 ---',
    EventData.length === 0,
  );
  console.log('--- EVENTS COMPONENT: !showLoading ---', !showLoading);
  console.log(
    '--- EVENTS COMPONENT: Will show empty component ---',
    EventData.length === 0 && !showLoading,
  );
  console.log('--- EVENTS COMPONENT: Will show loading ---', showLoading);
  console.log(
    '--- EVENTS COMPONENT: Will show FlatList ---',
    EventData.length > 0 && !showLoading,
  );
