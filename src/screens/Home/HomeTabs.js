...
const HomeTabs = props => {
  ...
  useEffect(() => {
    console.log(
      '--- HOME TABS: useEffect triggered - dispatching getUserGenresSagaAction ---',
    );
    if (userAccessToken) {
      console.log(
        '--- HOME TABS: Dispatching getUserGenresSagaAction with accessToken ---',
      );
      dispatch(getUserGenresSagaAction({accessToken: userAccessToken}));
    } else {
      console.log(
        '--- HOME TABS: No accessToken available, skipping getUserGenresSagaAction ---',
      );
    }
  ...
  const renderContaint = () => {
    console.log(
      '--- HOME TABS: renderContaint called with selectedTab ---',
      selectedTab,
    );
      ...
      <Chip
        ...
        onPress={() => {
          ...
          dispatch(
            currentScreenAction({
              ...screenData,
              selectedTabId: parseInt(item.id),
            }),
          );
        ...
        titleStyle={[
          ...
          {
            color:
              item.id === selectedTab ? Colors.radiumColour : Colors.labelColor,
          },
        ]}
      />
    ));
  ...
  return (
    <View style={{height: '100%'}}>
      <View style={styles.conatiner}>{renderToggleButtons()}</View>
      <View style={{height: '50%'}}>{renderContaint()}</View>
    </View>
  );
};
