...
const SignupValidators = (value, isPage1 = false) => {
  const baseSchema = {
    ...
    confirmPassword: yup
      .string()
      .trim()
      .oneOf([yup.ref('password'), null], strings('SignupValidators.confirmPasswordValid'))
    ...
    bandName: yup
      ...
      .when('artistCheck', {
        is: true,
        then: yup.string().required('Band/Artist name is required when registering as artist').max(50, ({ max }) => `Band name must be ${max} characters or less`),
