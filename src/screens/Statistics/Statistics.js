...
import {
  getUserStatistics,
  getRadioStationStatistics,
  getEventsStatistics,
  getGenresPrefrenceStatistics,
  getBandsStatistics,
  getPopularArtistStatistics,
...
const Statistics = () => {
  ...
  const getGenresPrefrenceStatisticsData = useSelector(
    getGenresPrefrenceStatistics,
  );
  const getPopularArtistStatisticsData = useSelector(
    getPopularArtistStatistics,
  );
  const getPopularArtistGenresStatisticsData = useSelector(
    getPopularArtistGenresStatistics,
  );
  ...
  const [genrePreferencePieChatData, setGenrePreferencePieChatData] =
    useState();
  const userStatisticsData = _.map(userStatistics, (data, index) => ({
    ...data,
    count: parseInt(data.count),
    color: index === 0 ? '#165D73' : '#1AA4CF',
  }));
  useEffect(() => {
    /* add key:value in array */
    const modifiedData = _.map(
      getGenresPrefrenceStatisticsData,
      (data, index) => ({
        ...data,
        name: data.genre,
        count: parseFloat(data.sum),
        color: genrePreferencePieChartColor[index],
      }),
    );
    const EventXaxisData = _.map(
      getEventsStatisticsData.months,
      (data, index) => ((index + 1) % 2 === 0 ? '' : data),
    );
  ...
  const showLoading = useSelector(
    state =>
      state.getEventsStatistics.isWaiting ||
      state.getUserStatistics.isWaiting ||
      state.getRadioStationStatistics.isWaiting ||
      state.getBandsStatistics.isWaiting ||
      state.getGenresPrefrenceStatistics.isWaiting ||
      state.getPopularArtistStatistics.isWaiting ||
      state.getPopularArtistGenresStatistics.isWaiting,
  );

  const isEmptyData =
    userStatisticsData &&
    userStatisticsData.length < 1 &&
    RadioStationStatisticsData === null &&
    genrePreferencePieChatData &&
    genrePreferencePieChatData.length < 1 &&
    getPopularArtistGenresStatisticsData &&
    getPopularArtistGenresStatisticsData.length < 1 &&
      ...
      <View style={{flexDirection: 'row', marginBottom: 20}}>
        ...
        <View
          ...
          <Image
            ...
            source={
              getPopularArtistStatisticsData.avatar
                ? {uri: getPopularArtistStatisticsData.avatar}
                : require('../../../assets/images/statisticsDefultUser.png')
            }
          />
        </View>
      </View>
            ...
            <View style={styles.imageBackgroundView}>
              <ImageBackground
                ...
                <View style={styles.genresNameView}>
                  <Text style={styles.genresNameTextStyle}>{item.name}</Text>
                </View>
              </ImageBackground>
              <Text style={styles.genresArtistTextStyle}>{item.artist}</Text>
            </View>
          )}
        />
      </View>
