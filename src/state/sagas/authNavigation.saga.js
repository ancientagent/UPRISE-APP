...
function* handleUserDetailsSuccess(action) {
  try {
    ...
    console.log(
      '--- AUTH NAVIGATION SAGA: Action payload ---',
    ...
    console.log(
      '--- AUTH NAVIGATION SAGA: Extracted user data ---',
      JSON.stringify(user, null, 2),
    );
    console.log(
      `--- AUTH NAVIGATION SAGA: onBoardingStatus is -> ${user?.onBoardingStatus} <-`,
    );

    if (user && typeof user.onBoardingStatus !== 'undefined') {
      if (user.onBoardingStatus === 0) {
        console.log(
          '--- AUTH NAVIGATION SAGA: DECISION -> INCOMPLETE ONBOARDING. Navigating to HomeSceneCreation.',
        );
        // For incomplete onboarding, go directly to Home Scene Creation
        RootNavigation.resetRoot('HomeSceneCreation');
      } else {
        console.log(
          '--- AUTH NAVIGATION SAGA: DECISION -> COMPLETE ONBOARDING. Navigating to Dashboard.',
        );
        RootNavigation.resetRoot('Dashboard');
      }
    } else {
      console.log(
        '--- AUTH NAVIGATION SAGA: ERROR - No onBoardingStatus found in user data ---',
      );
      console.log(
        '--- AUTH NAVIGATION SAGA: Available user keys ---',
        user ? Object.keys(user) : 'NO_USER_DATA',
      );
