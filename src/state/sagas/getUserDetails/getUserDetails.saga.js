...
export function* getUserDetailsWorkerSaga() {
  ...
  try {
    ...
    console.log('--- GET USER DETAILS SAGA: Token from Redux ---', userToken ? 'YES' : 'NO');
    
    // If no token in Redux, check AsyncStorage
    if (!userToken) {
      console.log('--- GET USER DETAILS SAGA: Checking AsyncStorage for token ---');
      userToken = yield call(TokenService.getAccessToken);
      console.log('--- GET USER DETAILS SAGA: Token from AsyncStorage ---', userToken ? 'YES' : 'NO');
    }
    
    console.log('--- GET USER DETAILS SAGA: Final token found ---', userToken ? 'YES' : 'NO');
    
    if (!userToken) {
      console.log('--- GET USER DETAILS SAGA: No token found anywhere - failing ---');
    ...
    console.log('--- GET USER DETAILS SAGA: API call completed ---', response ? 'SUCCESS' : 'FAILED');
    
    if (response !== null) {
      console.log('--- GET USER DETAILS SAGA: Fetch successful ---');
      console.log('--- GET USER DETAILS SAGA: User data from API ---', JSON.stringify(response.data.data, null, 2));
      ...
      console.log('--- GET USER DETAILS SAGA: Process complete (navigation handled by authNavigation saga) ---');
    } else {
