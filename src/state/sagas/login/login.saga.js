import {
  call, put, takeLatest, select, take, race,
...
export function* loginWorkerSaga(action) {
  ...
  try {
    ...
    if (response && response.data && response.data.user) {
        ...
        console.log('--- LOGIN SAGA: User profile direct from API ---', JSON.stringify(user, null, 2));
        ...
        console.log('--- LOGIN SAGA: Login process complete. Navigation will be handled by authNavigation saga. ---');
    } else {
        // Handle the case where the user object is missing
        console.error('--- LOGIN SAGA: ERROR - User data object not found in login response. ---', response?.data);
        yield put(loginRequestActions.fail('Login failed: Invalid response from server.'));
    }
} catch (error) {
