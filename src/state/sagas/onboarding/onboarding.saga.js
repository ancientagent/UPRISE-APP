...
import {
  showNetworkPopUp,
  hideNetworkPopUp,
} from '../../actions/network/Network.action';
...
function* validateCommunitySaga(action) {
  try {
    ...
    if (response && response.status === 'active') {
      ...
      console.log(
        'Community validation: status not active, staying on current screen',
      );
...
export default function* onboardingSaga() {
  yield takeLatest(
    OnboardingTypes.GET_SUPER_GENRES_REQUEST,
    getSuperGenresSaga,
  );
  yield takeLatest(
    OnboardingTypes.GET_CITY_SUGGESTIONS_REQUEST,
    getCitySuggestionsSaga,
  );
  yield takeLatest(
    OnboardingTypes.VALIDATE_COMMUNITY_REQUEST,
    validateCommunitySaga,
  );
}
