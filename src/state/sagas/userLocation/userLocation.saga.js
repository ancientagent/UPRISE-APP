...
export function* userLocationWorkerSaga(action) {
  ...
  try {
    ...
    console.log(
      '--- USER LOCATION SAGA: API Payload ---',
    ...
    console.log(
      '--- USER LOCATION SAGA: API Success Response ---',
    ...
    if (response !== null) {
      ...
      yield call(
        showAlert,
        'Welcome to your scene! Location and preferences saved successfully!',
      );

      console.log(
        '--- USER LOCATION SAGA: Location and station preference created. Fetching updated user details. ---',
      );
      ...
      console.log(
        '--- USER LOCATION SAGA: Auth navigation will handle routing to Dashboard ---',
      );
    }
  } catch (error) {
    console.error(
      '--- USER LOCATION SAGA: API Error ---',
